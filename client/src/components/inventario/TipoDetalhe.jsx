...
import ModalModelo from "./ModalModelo.jsx";

// "Página" de um tipo: marcas em linhas; clicar na marca expande os modelos;
// clicar no modelo abre o modal com todos os itens daquele modelo.
export default function TipoDetalhe({ grupo, onVoltar, setCardItem }) {
  const [marcasAbertas, setMarcasAbertas] = useState(() => new Set());
  const [selecionado, setSelecionado] = useState(null); // { marca, modelo }

  const Icone = iconeDoTipo(grupo.tipo);
              ...
              {aberta && (
                <div className="pb-2">
                  {marca.modelos.map((modelo, moi) => (
                    <button
                      key={`${chave}-${moi}`}
                      onClick={() => setSelecionado({ marca: marca.marca, modelo })}
