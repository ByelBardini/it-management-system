...
import { postMarca } from "../../services/api/marcaServices.js";
...
export default function AdicionarMarca({
  ...
  async function adicionaMarca() {
    if (nomeMarca == "") {
      ...
      setCarregando(true);
      try {
        await postMarca(nomeMarca, dominio, tipo, subtipo);

        setNotificacao({
          show: true,
          tipo: "sucesso",
          titulo: "Marca inserida com sucesso",
          mensagem: "A marca foi cadastrada e já pode ser usada nos cadastros.",
        });
        buscarDados();
      ...
      <div className="w-full max-w-md rounded-2xl bg-white/5 ring-1 ring-white/10 shadow-xl">
        <div className="flex items-center justify-between px-6 py-4 border-b border-white/10">
          <h2 className="text-lg font-semibold text-white">
            Adicionar Marca ({dominio === "peca" ? "Peças" : "Itens"})
          </h2>
          <button
        ...
        <div className="p-6 space-y-4">
          <div>
            <div className="flex items-center justify-between mb-1">
              <label className="text-sm text-white/70">Nome da Marca</label>
              <span className="text-xs text-white/50">{nomeMarca.length}/100</span>
            </div>
            <input
